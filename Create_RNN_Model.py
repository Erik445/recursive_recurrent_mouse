...
name_for_saved_model = "simpleRNN_model"
...
def read_CSV_File(file_path):
    # load dataset
    dataframe = pandas.read_csv(file_path, delimiter=',', skiprows=1, header=None, usecols=(24, 27, 28, 29, 31, 32, 33))
    ...
    width.extend(dataset[:, 3])
    time.extend(dataset[:, 0])
    target.extend(dataset[:, 2])
    sga.extend(dataset[:, 6])
    last_sgy.extend(dataset[:, 4])
    last_sgv.extend(dataset[:, 5])
...
for file in os.listdir(dir_path):
    ...
    train_X.extend(data_fit[0])
    train_y.extend(data_fit[1])

# MinMax Scaler to 0 - 1
#scaler = MinMaxScaler(feature_range=(0, 1))
#input_fit = scaler.fit_transform(input_fit)

sga_fit = np.reshape(train_y, (-1,1))
...
# design network
model = Sequential()
model.add(SimpleRNN(18, return_sequences= True,input_shape=(train_X.shape[1], train_X.shape[2])))
model.add(SimpleRNN(9, return_sequences= True))
model.add(SimpleRNN(3))
model.add(Dense(1))
model.compile(loss='mae', optimizer='adam')

# fit network
model.fit(train_X, train_y, epochs=40, batch_size=100, verbose=1)
